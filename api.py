...
import hmac
import hashlib
import urllib.parse  # ИЗМЕНЕНО: Импортируем urllib для надежного парсинга
...
async def get_locations(request):
    # Теперь отдаем и описание для новых карточек
    locations_list = [{"id": loc, "name": loc, "description": conf.get("description", "")} for loc, conf in
...
async def get_calendar_data(request):
    ...
    try:
        session = request.app['bot_app'].bot_data['aiohttp_session']
        today = date.today()
        # Запрашиваем данные на 60 дней, чтобы покрыть 2 месяца
        ...
        available_dates = [d.isoformat() for d, res in zip(dates_to_check, results) if res]
...
def setup_api_routes(app: Application, web_app: web.Application):
    ...
    web_app.router.add_get("/api/calendar", get_calendar_data)
