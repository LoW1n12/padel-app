...
import json
from typing import Dict, List, Optional  # ИЗМЕНЕНО: Добавлен импорт Dict и других нужных типов
...
async def get_vivacrm_sessions(session: aiohttp.ClientSession, check_date: date, court_config: dict) -> List[Dict]:
    """Получает данные о сеансах с API vivacrm."""
    params = {'date': check_date.strftime('%Y-%m-%d'), 'studioId': court_config['studioId'],
              'subServiceIds': court_config['subServiceIds'][0]}
    try:
        async with session.get(court_config['api_url'], params=params, headers=DEFAULT_HEADERS, timeout=15) as resp:
            if resp.status == 200:
                return await resp.json()
            logger.warning(f"VIVACRM: Статус {resp.status} для {check_date}")
            return []
    except Exception as e:
        logger.error(f"VIVACRM_EXCEPTION: Ошибка при запросе к {court_config['api_url']}: {e}", exc_info=True)
        return []
...
async def get_yclients_fili_sessions(session: aiohttp.ClientSession, check_date: date, location_config: dict,
    ...
    """Получает данные о сеансах с API yclients."""
    if not YCLIENTS_TOKEN:
        logger.warning("YCLIENTS: Токен авторизации (YCLIENTS_AUTH_TOKEN) не найден в .env файле.")
        return {}
    ...
    for slot in slots:
        ...
        if dt and dt.endswith(":00:00+03:00"):
            ...
            tasks.append(
                asyncio.create_task(session.post(url_services, headers=headers_with_auth, json=payload, timeout=15)))
            task_datetimes.append(dt)

    price_responses = await asyncio.gather(*tasks, return_exceptions=True)

    for i, resp_or_exc in enumerate(price_responses):
        if isinstance(resp_or_exc, Exception): continue
        resp = resp_or_exc
        if resp.status != 200:
            if resp.status == 401: logger.warning(
                f"YCLIENTS: Ошибка 401 для {task_datetimes[i]}. Токен авторизации, вероятно, истек.")
            continue
        try:
            services_data = await resp.json()
            time_iso = task_datetimes[i]
...
async def get_available_sessions_api(session: aiohttp.ClientSession, check_date: date, location: str,
    ...
    """Унифицированная функция для получения сеансов с любого API."""
    location_config = LOCATIONS_CONFIG.get(location, {})
    api_type = location_config.get('api_type')

    result_by_time = defaultdict(lambda: defaultdict(dict))
...
async def fetch_availability_for_location_date(session: aiohttp.ClientSession, location: str, check_date: date) -> Dict[
    str, Dict[str, Dict]]:
    """Собирает доступные сеансы для всех типов кортов на одной локации за одну дату."""
    location_config = LOCATIONS_CONFIG.get(location, {})
